...
public partial class SmartPlaylistSongListViewModel : SongListViewModelBase
{
    ...
    public async Task InitializeAsync(string title, Guid? smartPlaylistId, string? coverImageUri = null)
    {
        ...
        catch (Exception ex)
        {
            ...
            TotalItemsText = Resonance.WinUI.Resources.Strings.SmartPlaylist_ErrorLoading;
            Songs.Clear();
        }
    }
    ...
    private static string BuildRuleSummary(SmartPlaylist smartPlaylist)
    {
        ...
        if (ruleCount == 0)
            return Resonance.WinUI.Resources.Strings.SmartPlaylist_RuleSummary_NoRules;

        var matchType = smartPlaylist.MatchAllRules
            ? Resonance.WinUI.Resources.Strings.SmartPlaylist_MatchAll
            : Resonance.WinUI.Resources.Strings.SmartPlaylist_MatchAny;
        var ruleWord = ruleCount == 1
            ? Resonance.WinUI.Resources.Strings.SmartPlaylist_Rule_Singular
            : Resonance.WinUI.Resources.Strings.SmartPlaylist_Rule_Plural;

        return ResourceFormatter.Format(Resonance.WinUI.Resources.Strings.SmartPlaylist_RuleSummary_Format, matchType, ruleCount, ruleWord);
    }
