...
**Storage**: SQLite local via Entity Framework Core 10.0.11 (`Resonance.Core.Data.MusicDbContext`) com pool de conexões e migrations gerenciadas.
...
**Scale/Scope**: Solution composta por 4 projetos (`Resonance.WinUI`, `Resonance.Core`, `ResonanceAppFunctions`, `Resonance.Core.Tests`), ~40 suítes de teste de unidade e integração.
...
Resonance.sln
├── src/
│   ├── Resonance.Core/                   # Biblioteca central de lógica de domínio, serviços e modelos
│   │   ├── Constants/
...
│   └── Resonance.WinUI/                  # Interface do usuário em WinUI 3 / Windows App SDK
│       ├── Controls/
│       ├── Navigation/
│       ├── Pages/
...
│       └── Views/
├── tests/
│   └── Resonance.Core.Tests/             # Suíte de testes automatizados com xUnit v3 e MTP
└── api/
    └── ResonanceAppFunctions/            # Funções auxiliares Azure (opcionais / desacopladas)
```
...
- **Escopo ponta a ponta:** Fixação de upstream em tag `2.3.0` / commit `88b9790e` (HEAD `e242b8b0`), execução de `dotnet restore Resonance.sln -p:Platform=x64`, `dotnet build Resonance.sln -p:Platform=x64` e execução da suíte de testes.
- **Artefatos:** `contracts/toolchain-contract.md`, `research.md`.
- **Validação Local:** `dotnet restore Resonance.sln -p:Platform=x64` e compilação limpa.

### Slice 2: Architecture & Capability Audit
- **Meta Imutável:** *(mesma acima)*
- **Escopo ponta a ponta:** Mapeamento minucioso do grafo de DI em `App.xaml.cs`, dos serviços em `Resonance.Core.Services` e `Resonance.WinUI.Services`, e da persistência em `MusicDbContext`. Estabelecimento da regra de reutilização mandatória para impedir arquitetura concorrente.
